...
public class linkedMap {
	...
	//custom Node data
		...
		private void setZedMap(Node nextNode) {
			zedNode = nextNode;
		}
		
		private Node getZedMap() {
			return zedNode;
		}
	}
	...
	//create DFA conditional structure, traveling between nodes
		...
		node3.setZedMap(node12);
		node3.setSymbolMap(node12);
		...
		node5.setZedMap(node12);
		node5.setSymbolMap(node12);
		
		node6.setDigitMap(node5);
		node6.setUnderscoreMap(node6);
		
		node7.setAcceptance(true);
		node7.setZedMap(node12);
		node7.setSymbolMap(node12);
		...
		node10.setZedMap(node12);
		node10.setSymbolMap(node12);
		
		node11.setUnderscoreMap(node11);
		node11.setDigitMap(node10);
		
		node12.setAcceptance(true);
	}
	...
	public double analyzeString() {
		...
		int index = 0;
		while (!floatDone && index < inputChars.length) {
		//initializing objects
			nodeReturn checker = checkChar(inputChars[index], travNode);
			//go to next node based on the "checking" of the current node
			travNode = checker.getNode();
			
			//return an impossible value if no node is found (indicating no acceptance)
			if (travNode == null) {
			...
			case 0:
				if (checker.getChar() == 'd') {
					result = result*10 + getValue(inputChars[index]);
				}
			...
			case 3:
				if (checker.getChar() == 'd') {
					result = result + getValue(inputChars[index])*Math.pow(10, ((-1*decimalIndex) - 1));
					decimalIndex++;
				} else if (checker.getChar() == 'z' || checker.getChar() == 's') {
					floatDone = true;
					
					for (int i = index; i < inputChars.length; i++) {
						remainder = remainder + Character.toString(inputChars[i]);
					}
				}
				break;
			case 4:
			...
			case 7:
				...
				if (checker.getChar() == 'z' || checker.getChar() == 's') {
					floatDone = true;
			...
			case 10:
				if (checker.getChar() == 'd') {
					exponent = exponent*10 + getValue(inputChars[index]);
				} else if (checker.getChar() == 'z' || checker.getChar() == 's') {
					floatDone = true;
	...
	private nodeReturn checkChar(char input, Node currentNode) {
		...
		case '\u001E':
		case '\u001F':
			result = new nodeReturn(currentNode.getZedMap(), 'z');
			break;
		default:
